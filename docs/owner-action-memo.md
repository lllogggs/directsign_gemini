...
- Instagram DM 인증은 고객 Meta 로그인 없이 운영합니다. 운영자만 연락미 공식 Professional 계정 `@yeollockme`를 Meta App에 한 번 연결하고, 고객은 서버가 발급한 `DS-XXXX-XXXX` 코드를 공식 계정에 DM으로 보냅니다.
- Vercel 서버가 Meta 서명 webhook의 recipient와 sender username을 검증하고 제출 핸들·프로필 URL과 정확히 일치할 때만 자동 승인합니다. 운영자의 로컬 컴퓨터는 이 처리에 사용하지 않습니다.
- 실제 광고주 계정 1개로 사업자 인증부터 계약 작성, 공유 링크 발송까지 테스트합니다.
- 실제 인플루언서 계정 1개로 계약 링크 열기, 가입/로그인 후 복귀, Instagram DM 인증 요청, webhook 자동 승인, 서명까지 테스트합니다.
- 현재 production에는 Meta DM 필수 환경 변수와 관련 Supabase migration이 없으므로 자동 인증은 아직 live가 아닙니다. Meta App 권한·webhook·환경변수·migration·배포·실제 서명 E2E·Discord 실패 알림을 모두 확인한 뒤에만 운영 중으로 표시합니다.
- 전체 설정과 검증 순서는 [`instagram-dm-automation-runbook.md`](./instagram-dm-automation-runbook.md)를 따릅니다. 이전 수동 승인 또는 고객 Instagram/Facebook OAuth 지침은 사용하지 않습니다.
