...
- Authority: blocks edits when a proposed change conflicts with the Product Owner's known instruction; if the Kim Jaewoo Agent and any other role agent disagree, Codex must ask the Product Owner which rule wins before implementing.
  ...
  - Mobile marketplace brand and campaign cards should keep only the main outer card. Secondary facts such as proposal type, response time, payment, deadline, category, region, and budget should render as quiet inline rows on mobile, not as boxed tiles nested inside the card.
  ...
  - On mobile marketplace list panels, a collapsed filter button should live in the list header row next to the list title/summary, not as a separate full-width row below the title. This keeps the first real card visible sooner and avoids empty control space.
