...
- Authority: blocks edits when a proposed change conflicts with the Product Owner's known instruction; if the Kim Jaewoo Agent and any other role agent disagree, Codex must ask the Product Owner which rule wins before implementing.
  ...
  - For Chrome/Google-like dashboard tabs, do not draw every tab as an individual boxed segment. Inactive tabs should sit quietly on the tab strip, while only the active tab visually becomes the connected surface of the content panel below.
    ...
    - A completed production influencer signup must create a stable registered-member discovery identity so the creator appears in authenticated advertiser influencer discovery without a second public-profile-save step. Before platform verification, label the account honestly as unverified, keep platform handles and follower counts empty rather than inventing an `other` channel, keep creator country as `국가 미확인` without evidence, and do not expose that provisional identity as an indexed public profile or imply it is ready for a verified 1:1 proposal. Platform approval should enrich the same stable identity instead of replacing its handle or creating a duplicate. When an approved platform account claims an existing discovered identity, migrate organization interest records to the stable member handle, and never create an anonymous profile link until a real public profile is published. Interest save/unsave must resolve aliases and mutate the canonical identity atomically under the same transaction lock used by verification refresh, so concurrent stale tabs cannot recreate or strand an alias.
