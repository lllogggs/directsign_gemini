...
- Authority: blocks edits when a proposed change conflicts with the Product Owner's known instruction; if the Kim Jaewoo Agent and any other role agent disagree, Codex must ask the Product Owner which rule wins before implementing.
  ...
  - For Chrome/Google-like dashboard tabs, do not draw every tab as an individual boxed segment. Inactive tabs should sit quietly on the tab strip, while only the active tab visually becomes the connected surface of the content panel below.
    ...
    - Advertiser influencer discovery and campaign applicant selection must support channel-size sorting by subscribers/followers, and creator names/avatars should open that creator's public yeollock.me profile when a profile handle exists.
    - Paginated influencer discovery filters must be applied by the server before the page limit. Never fetch a mixed-platform first page and then present the client-filtered remainder as the filtered result; selecting a platform must return up to 1,000 matching creators first, then load the next matching page only when the actual list scroll area reaches the bottom.
    ...
    - Advertiser influencer saves belong to the advertiser organization, not the browser. The discovery list must expose one compact save control per creator and a saved-only filter backed by authenticated server data so the same advertiser account sees the same saved set across devices.
    ...
    - The advertiser discovery table order is 저장, 국가, 플랫폼, 인플루언서, 카테고리, 채널 지표, 액션. Keep the influencer-name column compact; use the cleaned category as useful middle information instead of restoring removed descriptions or filler copy.
    ...
    - Influencer public profile pages should stay simple and account-forward. Show the creator photo, name, concise intro, blue proposal CTA, and platform account buttons that open the influencer-registered accounts; do not add separate "대표 콘텐츠", portfolio/media-kit, or repeated channel/follower sections unless the Product Owner explicitly asks for richer profile content again.
    ...
    - Influencer public profile pages should look like a polished creator media-kit first page, not a sparse dashboard. Use one strong creator image, concise identity copy, a connected platform/follower stat strip, and one blue proposal CTA; avoid loose repeated cards that leave the page feeling empty.
    ...
    - Influencer public profile proposal areas should show only the blue "제안하기" button on the first screen. Do not add adjacent "제안", response-time, pricing, or helper labels in that action area unless the Product Owner explicitly asks for them again.
    ...
    - Marketplace and dashboard-like filters should use compact expandable list rows instead of always-visible checkbox/chip groups. Show the filter name and current summary first, expand only the selected filter into a vertical list, allow multi-select where the data model needs it, and label country filters simply as "국가" instead of "오디언스 국가" or "타깃 국가".
...
- Campaign creation rules:
  ...
  - 서포터즈 캠페인은 제품 제공을 전제로 글/게시 미션을 수행하는 캠페인 유형이다. 계약 초안에는 제품 재판매 금지, 재판매 적발 시 서포터즈 활동 자동 박탈 및 제품 제공비 청구, 콘텐츠 게시 유지, 미션 불이행 시 제품 제공비 청구 가능 조항이 포함되어야 한다.
