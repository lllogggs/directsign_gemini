...
- QA and browser verification rules:
  ...
  - Advertiser sales PDF pages must keep a consistent top start line for the left text block and the right visual/dashboard area across all pages. Do not let page-specific screenshot margins make one page begin higher or lower unless the Product Owner explicitly approves that exception.
  ...
  - If the Product Owner later asks to adjust only the left text vertical position, split the sales PDF copy start from the visual/dashboard start. Keep the right product area fixed and move only the left copy block.
