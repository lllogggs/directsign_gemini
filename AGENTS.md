...
- Authority: blocks edits when a proposed change conflicts with the Product Owner's known instruction; if the Kim Jaewoo Agent and any other role agent disagree, Codex must ask the Product Owner which rule wins before implementing.
  ...
  - Advertiser intro PC proposal pages after the first slide must preserve the approved dense, real-service dashboard feel. Do not replace the PC writing, contract-management, or applicant slides with sparse centered text, tiny floating documents, empty dashboard panels, or hand-built mock dashboards. Use actual service capture assets for the PC writing, full content-review/contract-management, and campaign-applicant previews; avoid sparse one-submission review captures when a fuller real review/completed capture exists. The campaign-applicant capture must visibly show the full 12-applicant operating list when the copy says 12 applicants.
