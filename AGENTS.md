...
- Intro and landing rules:
  ...
  - Advertiser intro previews should show the real contract dashboard states "작성중", "진행중", "종료"; influencer intro previews should show "지원중", "진행중", "완료", "미선정".
...
- QA and browser verification rules:
  ...
  - Advertiser sales PDF campaign-applicant screenshots must feel full enough for a proposal. When applicant data exists, choose the campaign with the most visible selectable influencer rows first; do not use a sparse one- or two-row applicant list that makes the service look empty.
