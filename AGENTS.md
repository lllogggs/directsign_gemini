...
- Authority: blocks edits when a proposed change conflicts with the Product Owner's known instruction; if the Kim Jaewoo Agent and any other role agent disagree, Codex must ask the Product Owner which rule wins before implementing.
  ...
  - Kim Jaewoo review standard:
    ...
    - Influencer contract share-link 산출물 summaries should surface advertiser-entered values as emphasized facts. On the first summary screen, do not use "산출물" as a parent row; split it into separate "플랫폼", "컨텐츠", and "수량" rows so the influencer can scan the actual advertiser-entered terms immediately. If advertiser-added special terms exist, show a conditional "특약" row on the first summary and a "특약사항" detail row after opening the contract; hide both when no special terms exist. Deeper contract-detail sections may still use the 산출물 grouping when the surrounding context is already detailed.
  ...
  - Influencer intro pages must now lead like the advertiser pain-point opener: "광고계약 / 흩어진 광고 계약은 / 위험합니다." with visual risk cards such as 광고비 미지급, 마감일 착오, 컨텐츠 기준 변경, and 활용 범위 과다. The real received-contract dashboard should follow immediately after the pain point, not replace the opener.
