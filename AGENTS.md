...
- Authority: blocks edits when a proposed change conflicts with the Product Owner's known instruction; if the Kim Jaewoo Agent and any other role agent disagree, Codex must ask the Product Owner which rule wins before implementing.
  ...
  - Blue primary CTAs were directly requested by the Product Owner as a product-wide rule. Primary forward actions across the service, such as "시작하기", "새 계약", "제안", "서명하기", submit/save, and other main CTAs, must use the approved blue CTA hierarchy unless the Product Owner explicitly requests a specific exception.
  ...
  - Advertiser intro PC proposal pages after the first slide must preserve the approved dense, real-service dashboard feel. Do not replace the PC writing, link-share, contract-management, or applicant slides with sparse centered text, tiny floating documents, empty dashboard panels, montages, or hand-built mock dashboards. Use headerless, focused actual service capture assets for the PC writing, link-share, content-review/contract-management, and campaign-applicant previews; do not include the app/global service header inside the intro capture. Focused capture crops must keep sidebars and action panels fully visible with enough breathing room that the service screen does not look cut off at the sides. If contain-fit leaves vertical space, center the capture by height and use the neutral gray intro background behind it rather than white top/bottom letterbox bands. The link-share preview must show the actual "공유 링크 생성" action, the content-review capture must match 콘텐츠 확인/수정요청 copy, and the campaign-applicant capture must visibly show the full 12-applicant operating list when the copy says 12 applicants.
  ...
  - Influencer intro pages must answer why a creator would use the product instead of email or Kakao. First viewport copy should surface concrete creator value such as advertiser-entered terms, revision-request records, and signed-document storage before dashboard taxonomy copy.
